...
valModelName = ['Model_CEDN_Separation_S1_NSGA_II_9_test_128.h5',
                'Model_CEDN_Separation_S2_NSGA_II_9_test_128.h5']
valWeightName = ['Weight_Temp_Separation_S1_NSGA_II_9_test_128_datX.h5',
...
def funTestPlot2(Xfname, Yfname, testName, parThreshold=0.95, CEDN=model):
    imgX = image.load_img(Xfname, color_mode='grayscale')
    imgTemp = image.load_img(Yfname, color_mode='grayscale')
    imgX = image.img_to_array(imgX)
    imgTemp = image.img_to_array(imgTemp)
    ...
    plt.plot(x2, y2, '+', ms=8, label='Theoretical', color='r')
    plt.plot(x1, y1, 'o', ms=8, label='Predicted', color='b',
             markerfacecolor='none')
    plt.legend()
    plt.title('POF({})'.format(testName))
...
"""
for i in ['ZDT1', 'ZDT2', 'ZDT3', 'ZDT4', 'ZDT6', 'POL', 'KUR', 'FON', 'SCH']:
    print('=======    {}    ======='.format(i))
    Xfname = './datSet/NSGA_II_9_test/datX/imgX_{}.bmp'.format(i)
    Yfname = './datSet/NSGA_II_9_test/datZ/imgZ_{}.bmp'.format(i)
    funTestPlot(Xfname, Yfname, 0.5)
    funTestPlot2(Xfname, Yfname, i, 0.5)
