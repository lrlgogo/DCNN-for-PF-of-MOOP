...
valModelName = ['Model_CEDN_128_d_1.h5', 'Model_CEDN_128_d_2.h5']
valWeightName = ['Weight_ZDT123_d_1.h5', 'Weight_ZDT123_d_2.h5']
valImageSize = (2**7, 2**7)

model1 = models.load_model('.\{}'.format(valModelName[0]))
model1.load_weights('.\{}'.format(valWeightName[0]))
model1.name += '_model1'
for layer in model1.layers:
    layer.name += '_model1'
model1.summary()
model2 = models.load_model('.\{}'.format(valModelName[1]))
model2.load_weights('.\{}'.format(valWeightName[1]))
model2.name += '_model2'
...
for i in range(10):
    Xfname = './datSet/ZDT/ZDT123/datX1d/imgX_{}.bmp'.format(5000 + i)
    Yfname = './datSet/ZDT/ZDT123/datZ1d/imgZ_{}.bmp'.format(5000 + i)
    funTestPlot(Xfname, Yfname)

for i in ['ZDT1', 'ZDT2', 'ZDT3']:
    print('=======    {}    ======='.format(i))
    Xfname = './datSet/ZDT/ZDT123/datX1d/imgX_{}.bmp'.format(i)
    Yfname = './datSet/ZDT/ZDT123/datZ1d/imgZ_{}.bmp'.format(i)
    funTestPlot(Xfname, Yfname)
