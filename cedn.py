...
"""===   Global Parameters   ============================================="""
valName = 'ZDT4'
valTrainFileName = ['datXd5', 'datZd5']
valImageSize = (2**7, 2**7)
valNumDat = 5*1024#1*5120#2048 * 5 * 1#2 * 4096
valDatDir = './datSet/ZDT/{}'.format(valName)#ZDT/ZDT4'#'./datSet/ZDT4'
valTrainEpochs = 120
valBatchSize = 32
valWeightFile = './datModels/Weight_Intergrant_{0}_{1}_{2}.h5'.format(
...
for i in range(valNumDat):
    ...
    Yfname = os.path.join(varTrainYDir, 'imgZ_{}.bmp'.format(i))#Y or Z
...
#varPar1, varPar2 = [800, 912]#[716, 870]#[102, 115]#[512, 768]
...
"""===   Train Setting   ================================================="""
callbacks_list = [
    keras.callbacks.EarlyStopping(monitor='acc', patience=30),
...
CEDN.save_weights('./datModels/Weight_Temp_Intergrant_{0}_{1}_{2}.h5'.format(
    valName, valImageSize[0], valTrainFileName[0]))
CEDN.save('./datModels/Model_CEDN_Intergrant_{0}_{1}.h5'.format(
...
def funTestPlot(parThreshold=0.95,
    ...
    for j in range(valImageSize[1]):
        ...
        if varTemp > parThreshold:
            imgY[varIndex][j] = 1.01
        else:
            imgY[varIndex][j] = -0.01
