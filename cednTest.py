...
def funTestPlot(parXFName, parYFName,
        ...
        for j in range(np.shape(imgY)[1]):
            ...
            if imgTemp[np.shape(imgY)[0] - 1 - i][j] == 0:
                ...
                metYReal.append((j / float(parImageSize[1]),
                                 i / float(parImageSize[0])))
    plt.subplot(223)
    plt.plot(x2, y2, 'bo', ms=1, label='Theoritical PF')
    x1_temp = [x1[2 * i] for i in range((len(x1) - 1) // 2 + 1)]
    y1_temp = [y1[2 * i] for i in range((len(y1) - 1) // 2 + 1)]
    plt.plot(x1_temp, y1_temp, 'rx', ms=4, label='Predictive PF')
    plt.legend()
    plt.title('theoretical and predictive PF')
    ...
    plt.title('predictive PF')
    plt.subplot(224)
    plt.imshow(imgTemp, cmap='Greys_r')
    plt.title('theoretical PF')
